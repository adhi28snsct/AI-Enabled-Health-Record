...
export default function DoctorPortal() {
  ...
  // Keep using localStorage userId as you already do
  ...
  // --- Load doctor profile from server and normalize fields ---
  const loadDoctorProfile = useCallback(async () => {
    try {
      const storedDoctorId = localStorage.getItem('userId');
      if (!storedDoctorId) return null;

      const res = await api.get(`/doctor/doctors/${storedDoctorId}`);
      const payload = res?.data?.data ?? res?.data ?? res;

      const normalized = {
        _id: payload._id ?? storedDoctorId,
        name: payload.name ?? "",
        phone: payload.phone ?? "",
        dob: payload.dob ?? "",
        // server uses blood_type; propagate into UI key blood_group
        blood_group: payload.blood_type ?? payload.blood_group ?? "",
        specialization: payload.specialization ?? payload.otherSpecialization ?? "",
        otherSpecialization: payload.otherSpecialization ?? ""
      };

      setDoctorProfile(prev => ({ ...prev, ...normalized }));

      // keep localStorage consistent for other app places that read it
      localStorage.setItem("name", normalized.name || "");
      localStorage.setItem("phone", normalized.phone || "");
      localStorage.setItem("dob", normalized.dob || "");
      localStorage.setItem("blood_group", normalized.blood_group || "");
      localStorage.setItem("specialization", normalized.specialization || "");
  ...
  // load profile on mount so header badge/profile shows server values after refresh
  useEffect(() => {
    (async () => {
  ...
  const handleDoctorSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        // map UI blood_group → server schema blood_type
        blood_type: doctorProfile.blood_group,
        specialization: finalSpecialization,
      };
      const id = doctorProfile._id || localStorage.getItem("userId");
      ...
      const res = await api.patch(`/doctor/doctors/${id}`, payload);
      const updated = res?.data?.data ?? res?.data ?? res;

      // Update UI quickly from server response (map blood_type -> blood_group)
      if (updated) {
        setDoctorProfile(prev => ({
          ...
          blood_group: updated.blood_type ?? updated.blood_group ?? prev.blood_group,
          specialization: updated.specialization ?? prev.specialization,
          otherSpecialization: updated.otherSpecialization ?? prev.otherSpecialization
        }));
        ...
        localStorage.setItem("blood_group", updated.blood_type ?? updated.blood_group ?? "");
        localStorage.setItem("specialization", updated.specialization ?? "");
      }

      // authoritative sync to ensure any backend-side normalization is reflected
      await loadDoctorProfile();
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-50 border-b">
        <div className="px-4 py-4 flex items-center justify-between">
          ...
          {/* Right side: doctor avatar/name + appointments panel */}
          <div className="flex items-center gap-3">
            {/* Appointments panel (bell with confirmed count) */}
            <DoctorAppointmentsPanel doctorId={localStorage.getItem("userId")} />
            ...
            <div
              ...
              role="button"
              tabIndex={0}
              // removed outer onClick to avoid duplication; use the button below for opening modal
            >
              <svg
